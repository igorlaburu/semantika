...
class SemanticSearchRequest(BaseModel):
    ...
    threshold: float = Field(default=0.2, ge=0.0, le=1.0, description="Minimum similarity score (0.0-1.0, default 0.2 for broader matches)")
...
class CreateArticleRequest(BaseModel):
    ...
    contenido: str = Field(..., description="Article content (HTML)")
    excerpt: Optional[str] = Field(None, description="Article excerpt/summary")
    autor: str = Field(default="Sistema", description="Author name")
    ...
    estado: str = Field(default="borrador", description="Article status (borrador/publicado)")
    imagen_url: Optional[str] = Field(None, description="Featured image URL")
    news_ids: Optional[List[str]] = Field(None, description="Related news context unit IDs")
    working_json: Optional[Dict[str, Any]] = Field(None, description="Working draft JSON")
...
async def create_article(
    ...
    """Create a new article (borrador or publicado)."""
    try:
        import uuid
        from datetime import datetime
        ...
        article_data = {
            ...
            "working_json": request.working_json,
            "created_at": now,
            "updated_at": now
        }
        
        if request.estado == "publicado":
            article_data["fecha_publicacion"] = now
        
        result = supabase.client.table("press_articles").insert(article_data).execute()
        
        if result.data and len(result.data) > 0:
            logger.info("article_created",
                article_id=article_id,
                company_id=company_id,
                titulo=request.titulo[:50],
                estado=request.estado
            )
            return result.data[0]
        else:
            logger.error("article_creation_failed_no_data", titulo=request.titulo[:50])
            raise HTTPException(status_code=500, detail="Failed to create article")
        
    except HTTPException:
        raise
    except Exception as e:
        logger.error("create_article_error", error=str(e), titulo=request.titulo[:50])
