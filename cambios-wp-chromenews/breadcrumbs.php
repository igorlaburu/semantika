...
class Breadcrumb_Trail
{
    ...
    protected function add_day_archive_items()
    {
		...
		$month = sprintf($this->labels['archive_month'], date_i18n('F', get_the_time('U'))); //igor: reescribo
        $day   = sprintf($this->labels["archive_day"],   get_the_time(esc_html_x("d", "daily archives date format",   "chromenews")));
    ...
    protected function add_month_archive_items()
    {
        ...
        $year  = sprintf($this->labels['archive_year'],  get_the_time(esc_html_x('Y', 'yearly archives date format',  'chromenews')));
        $month = sprintf($this->labels['archive_month'], get_the_time(esc_html_x('F', 'monthly archives date format', 'chromenews')));
		$month = sprintf($this->labels['archive_month'], date_i18n('F', get_the_time('U')));//igor
    ...
    protected function map_rewrite_tags($post_id, $path)
    {
        ...
        if (is_array($matches)) {
            ...
            foreach ($matches as $match) {
                ...
                elseif ('%monthnum%' == $tag)
                    /*$this->items[] = sprintf('<a href="%s">%s</a>', esc_url(get_month_link(get_the_time('Y', $post_id), get_the_time('m', $post_id))), sprintf($this->labels['archive_month'], get_the_time(esc_html_x('F', 'monthly archives date format', 'chromenews')))); igor*/
				   $this->items[] = sprintf('<a href="%s">%s</a>', esc_url(get_month_link(get_the_time('Y', $post_id), get_the_time('m', $post_id))), sprintf($this->labels['archive_month'], date_i18n('F', get_the_time('U', $post_id))));


                // If using the %day% tag, add a link to the daily archive.
                elseif ('%day%' == $tag)
					$this->items[] = sprintf('<a href="%s">%s</a>', esc_url(get_day_link(get_the_time('Y', $post_id), get_the_time('m', $post_id), get_the_time('d', $post_id))), sprintf($this->labels['archive_day'], get_the_time(esc_html_x('d', 'daily archives date format', 'chromenews'))));
